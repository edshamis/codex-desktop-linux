...
function linuxDesktopReadAloudSettingsSource() {
  return [
    ...
    `constructor(e){super(e),this._alive=!1,this._enabledWrite=0,this._speedWrite=0,this.state={enabled:!1,speed:1.05,isLoading:!0,error:null},this.load=this.load.bind(this),this.updateEnabled=this.updateEnabled.bind(this),this.updateSpeed=this.updateSpeed.bind(this)}`,
    `componentDidMount(){this._alive=!0,this.load()}`,
    `componentWillUnmount(){this._alive=!1}`,
    `load(){this.setState({isLoading:!0}),Promise.allSettled([__post("get-global-state",{params:{key:KEYS.readAloud}}),__post("get-global-state",{params:{key:KEYS.readAloudSpeed}})]).then(([e,t])=>{if(!this._alive)return;let n=e.status==="fulfilled",r=t.status==="fulfilled",i=e.status==="rejected"?e.reason:t.status==="rejected"?t.reason:null;this.setState({enabled:n?e.value?.value===!0:this.state.enabled,speed:r?codexLinuxReadAloudPaceValue(t.value?.value??1.05):this.state.speed,error:i==null?null:i instanceof Error?i.message:String(i)})}).finally(()=>{this._alive&&this.setState({isLoading:!1})})}`,
    `updateEnabled(e){let t=codexLinuxChecked(e),n=this.state.enabled,r=++this._enabledWrite;this.setState({enabled:t,error:null}),__post("set-global-state",{params:{key:KEYS.readAloud,value:t}}).catch(e=>{this._alive&&r===this._enabledWrite&&this.setState({enabled:n,error:e instanceof Error?e.message:String(e)})})}`,
    `updateSpeed(e){let t=codexLinuxReadAloudPaceValue(e?.currentTarget?.value??e),n=this.state.speed,r=++this._speedWrite;this.setState({speed:t,error:null}),__post("set-global-state",{params:{key:KEYS.readAloudSpeed,value:t}}).catch(e=>{this._alive&&r===this._speedWrite&&this.setState({speed:n,error:e instanceof Error?e.message:String(e)})})}`,
