...
test("local feature stays disabled until explicitly enabled", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "quick-chat-window-zoom-"));
  try {
...
test("applies zoom to a popped-out Quick Chat viewport", () => {
  const source = `c0={zoomedViewport:a0,floatingSurface:o0};root=${unpatchedRoot()}`;
...
test("patches every verified Quick Chat root", () => {
  const source = `c0={zoomedViewport:a0,floatingSurface:o0};first=${unpatchedRoot()};second=${unpatchedRoot()}`;
  const patched = applyPatchTwice(source);
  assert.equal(
...
test("patches an unpatched root beside an already patched root", () => {
  const source = `c0={zoomedViewport:a0,floatingSurface:o0};first=${patchedRoot()};second=${unpatchedRoot()}`;
  const patched = applyPatchTwice(source);
  assert.equal(
    (patched.match(/c===`window`&&vC\(c0\.zoomedViewport/g) ?? []).length,
    2,
...
test("does not mistake an unrelated zoomed viewport for Quick Chat", () => {
  const source = `unrelated=x===\`window\`&&classes(s0.zoomedViewport,\`relative overflow-hidden bg-token-editor-background/55\`);c0={zoomedViewport:a0,floatingSurface:o0};quick=${unpatchedRoot()}`;
  const patched = applyPatchTwice(source);
  assert.match(patched, /quick=c===`floating`[^;]+c===`window`&&vC\(c0\.zoomedViewport/);
});

test("rejects a missing style contract", () => {
  const source = `unrelated=vC(c0.zoomedViewport,\`relative\`);quick=${unpatchedRoot()}`;
  assert.deepEqual(
    captureWarnings(() => assert.equal(applyQuickChatWindowZoomPatch(source), source)),
    [warning],
  );
});

test("does not bind a style identifier suffix", () => {
  const source = `ba={zoomedViewport:a0,floatingSurface:o0};quick=${unpatchedRoot("c", "vC", "a")}`;
  assert.deepEqual(
    captureWarnings(() => assert.equal(applyQuickChatWindowZoomPatch(source), source)),
    [warning],
  );
...
test("rejects a style contract outside the bounded lookbehind", () => {
  const source = `c0={zoomedViewport:a0,floatingSurface:o0}${";".repeat(12_001)}quick=${unpatchedRoot()}`;
  assert.deepEqual(
    captureWarnings(() => assert.equal(applyQuickChatWindowZoomPatch(source), source)),
    [warning],
  );
...
test("validates the style contract for already patched roots", () => {
  const source = `c0={zoomedViewport:a0,floatingSurface:o0}${";".repeat(12_001)}quick=${patchedRoot()}`;
  assert.deepEqual(
    captureWarnings(() => assert.equal(applyQuickChatWindowZoomPatch(source), source)),
    [warning],
  );
...
test("leaves every root unchanged when one root contract is missing", () => {
  const source = `c0={zoomedViewport:a0,floatingSurface:o0};first=${unpatchedRoot()};second=${unpatchedRoot("d", "wC", "d0")}`;
  assert.deepEqual(
    captureWarnings(() => assert.equal(applyQuickChatWindowZoomPatch(source), source)),
    [warning],
  );
...
test("warns on a recognizable but drifted Quick Chat root", () => {
  ...
  assert.deepEqual(captureWarnings(() => applyQuickChatWindowZoomPatch(source)), [
    warning,
  ]);
});
